...
# Note: time.time() depends on clock, not exact
class HostapdHandler():
    def __init__(self, wlan_device="wlan0", init_ssid="SmartAP"):
        self.last_restart_time = time.time()-100
        self.wlan_device = wlan_device
        self.change_wifi_ssid(init_ssid)

    def restart_min_interval(self):
        """hostapd cant be restarted too often """
    ...
    def restart_hostapd(self):
        """Always call this after updating the config file
    ...
    def change_wifi_password(self, psk):
        ...
        for line in fileinput.input(hostapd_conf_path, inplace=True): 
